...
BEGIN
    ...
    WHERE (sup.phone_number = p_support_contact OR sup.email = p_support_contact) AND r.status = 'cancelled';
END;
$$ LANGUAGE plpgsql;
...
RETURNS TABLE(ticket_id INT, match_title TEXT, venue_name VARCHAR, match_date TIMESTAMP) AS $$
BEGIN
    RETURN QUERY
    SELECT t.ticket_id, (t.home_team || ' vs ' || t.away_team) AS match_title, t.venue_name, t.match_date
    FROM tickets t
    JOIN reservations r ON t.ticket_id = r.ticket_id
...
BEGIN
    ...
    SELECT 'Ticket'::TEXT AS source_type, ticket_id AS item_id, 
           (home_team || ' vs ' || away_team || ' at ' || venue_name || ' (' || ticket_tier || ')') AS description
    FROM tickets
...
RETURNS TABLE(neighbor_id INT, neighbor_name TEXT, email VARCHAR, city VARCHAR) AS $$
DECLARE
    v_user_city VARCHAR;
BEGIN
    ...
    RETURN QUERY
    SELECT u.user_id, (u.first_name || ' ' || u.last_name) AS neighbor_name, u.email, u.city
    FROM users u
...
CREATE OR REPLACE FUNCTION get_top_buyers_since_date(p_since_date TIMESTAMP, p_top_n INT)
RETURNS TABLE(user_id INT, full_name TEXT, total_spent NUMERIC) AS $$
BEGIN
    RETURN QUERY
    SELECT u.user_id, (u.first_name || ' ' || u.last_name) AS full_name, SUM(p.amount) AS total_spent
    FROM users u
...
DROP FUNCTION IF EXISTS get_cancelled_reservations_by_sport(sport_type_enum) CASCADE;
CREATE OR REPLACE FUNCTION get_cancelled_reservations_by_sport(p_sport_type sport_type_enum)
RETURNS TABLE(reservation_id INT, ticket_title TEXT, cancelled_at TIMESTAMP) AS $$
BEGIN
    RETURN QUERY
...
DROP FUNCTION IF EXISTS get_users_with_most_reports_by_category(VARCHAR) CASCADE;
...
-- 9. Calculate Cancellation Penalty & Refund Amount (Anti-Infinite-Money-Glitch Patch)
...
BEGIN
    -- 1. Fetch exact paid amount from payments table (NOT current dynamic ticket price)
    SELECT t.match_date, p.amount, r.status::VARCHAR
    INTO v_match_date, v_paid_amount, v_status
    FROM reservations r
    JOIN tickets t ON r.ticket_id = t.ticket_id
    LEFT JOIN payments p ON p.reservation_id = r.reservation_id AND p.status = 'successful'
