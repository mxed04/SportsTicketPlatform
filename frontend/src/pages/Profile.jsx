...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 🚀 FIXED: Removed N+1 fetch loop. Data now directly from backend.
        const enriched = bookingsList.map((b) => {
          ...
          return { 
            ...b, 
            fetchedPrice: b.final_price || 0, 
            fetchedVenue: b.venue_name || 'Stadium', 
        ...
        enriched.sort((a, b) => {
          ...
          const isFutA = dateA >= now;
          const isFutB = dateB >= now;

          if (isFutA && isFutB) return dateA - dateB;
          if (!isFutA && !isFutB) return dateB - dateA;
          return isFutA ? -1 : 1;
        });
  ...
  const handleCancelTicket = async (reservationId, status) => {
    ...
    const msgPend = 'Cancel this unpaid reservation?';
    const msgPaid = 'Cancellation includes a penalty fee. Continue?';
    ...
    try {
      ...
      if (!isPending && refund !== undefined && refund > 0) {
        successMsg = `Ticket cancelled. ${
          Number(refund).toLocaleString()
        } Toman refunded to your account.`;
      }
      
      toast.success(successMsg, { duration: 6000 });
      setBookings(prev => prev.map(b => 
        (b.reservation_id || b.id) === reservationId 
          ? { ...b, effectiveStatus: 'cancelled' } 
          : b
      ));
    } catch (error) {
      const msg = error.response?.data?.detail || 'Failed to cancel.';
  ...
  const handleResumePayment = (booking) => {
    ...
    const title = (booking.home_team && booking.away_team) 
      ...
      : (booking.title || 'Event Ticket');
    
    navigate(`/payment/${resId}`, { 
      state: { 
        finalPrice: booking.fetchedPrice || 0, 
        title, 
  ...
  const inputClass = 
    ...
    "focus:outline-none focus:border-indigo-500 text-sm text-white " +
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-gray-100 font-sans pb-16">
        ...
        >
          ...
          <div className="flex gap-4">
            <button 
              ...
              className={
                "text-xs font-semibold text-gray-300 hover:text-white " +
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 pt-8">
        <div 
          className={
            "flex justify-between items-center bg-white/5 backdrop-blur-xl " +
            "p-6 rounded-3xl border border-white/10 shadow-2xl mb-8"
          }
        >
          <div className="flex items-center gap-4">
            <div 
              className={
                "w-16 h-16 bg-gradient-to-tr from-indigo-500 to-blue-500 " +
                "text-white rounded-2xl flex items-center justify-center " +
                "text-2xl font-bold shadow-lg shadow-indigo-500/20"
              }
            >
          ...
          <button 
            ...
            className={
              ...
              "border-rose-500/20 text-rose-400 font-bold rounded-xl " +
        ...
        <div className="flex gap-4 mb-8 border-b border-white/10 pb-px">
          <button 
            ...
            className={
              "pb-3 px-2 font-bold text-xs uppercase tracking-wider " +
        ...
        {activeTab === 'bookings' && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              <div 
                className={
                  "bg-white/5 backdrop-blur-xl p-5 rounded-2xl border " +
                  "border-white/10"
                }
              >
                <span 
            ...
            <div 
              className={
                "bg-white/5 backdrop-blur-xl rounded-3xl border " +
                "border-white/10 p-6 mb-8 shadow-2xl"
              }
            >
              {loading ? (
                <div 
                  className={
                    "text-center py-12 text-gray-500 animate-pulse text-xs"
                  }
                >
                ...
                <div 
                  className={
                    "text-center py-16 text-gray-400 bg-black/20 " +
                  ...
                  {bookings.map((booking, index) => {
                    ...
                    return (
                      <div 
                        key={bookingId || index} 
                        className={
                          "flex flex-col border p-5 rounded-2xl " +
                          "transition-all " +
                          (status === 'pending' 
                      ...
                      >
                          ...
                          >
                            {status === 'pending' && (
                              <button 
                                ...
                                className={
                                  ...
                                  "tracking-wider animate-pulse shadow-lg " +
                            ...
                            {(status === 'paid' || status === 'pending') && (
                              <button 
                                ...
                                className={
                                  "w-full sm:w-auto px-5 py-2.5 bg-white/5 " +
                                  "border border-rose-500/30 text-rose-400 " +
                                  "rounded-xl hover:bg-rose-500/10 font-bold " +
                        ...
                        {status === 'paid' && booking.qr_code && (
                          <div 
                            className={
                              "mt-5 pt-4 border-t border-white/10 flex " +
                              "flex-col sm:flex-row items-center gap-5 " +
                              "bg-white/[0.02] p-4 rounded-xl"
                            }
                          >
                            <img 
                              src={booking.qr_code} 
                              alt="Ticket QR" 
                              className={
                                "w-20 h-20 rounded-xl bg-white p-1.5 shadow-lg"
                              } 
                            />
                            <div className="text-center sm:text-left">
                              ...
                              >
                                Official E-Ticket / Bank Ref
                              </span>
                              <span 
                                className={
                                  "text-sm font-mono font-bold text-white " +
                                  "tracking-widest bg-white/5 px-3 py-1.5 " +
                              ...
                              <p className="text-[9px] text-gray-500 mt-2">
                                Present this QR code at the stadium gates.
                              </p>
                            </div>
                          </div>
                        )}
        ...
        {activeTab === 'settings' && (
          <div 
            className={
              "bg-white/5 backdrop-blur-xl rounded-3xl border " +
              "border-white/10 p-8 shadow-2xl mb-8"
            }
          >
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label 
                    className={
                      "block text-xs font-bold text-gray-400 mb-1.5 " +
                ...
                <div className="md:col-span-2">
                  <label 
                    className={
                      "block text-xs font-bold text-gray-500 mb-1.5 " +
                      "uppercase tracking-wider"
                    }
                  >
                    Phone Number (Locked)
                  ...
                  <input 
                    type="text" 
                    className={
                      "w-full px-4 py-3 bg-white/[0.02] border " +
                      "border-white/5 rounded-xl text-sm text-gray-500 " +
                      "cursor-not-allowed"
                    } 
              ...
              <div className="pt-4 flex justify-end">
                <button 
                  ...
                  className={
                    "px-8 py-3.5 bg-gradient-to-r from-indigo-600 " +
                    "to-blue-600 hover:from-indigo-500 hover:to-blue-500 " +
